# Dont forget to create 'test' folder in model directory
...
def decoder_block(input1, input2, feature, kernel, dropout):
    ...
    c = Conv2D(feature, (kernel, kernel), activation='relu', kernel_initializer='he_normal', padding='same')(u)
    c = Dropout(dropout)(c)
    c = Conv2D(feature, (kernel, kernel), activation='relu', kernel_initializer='he_normal', padding='same')(c)
    return c

################################################################
def simple_unet_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS, kernel_size):
    # Build the model
...
################################################################
def get_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS, kernel_size):
    return simple_unet_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS, kernel_size)

################################################################
def run_trials(data_directory, model_directory, kernel_size, image_magnification, type_model):
    # kernel_size = 7
    # trial_num = 2 # for specific trial num
    # image_magnification = 'x4'
    
    type_model = 'unet_%s' % kernel_size
    ...
    save_model = model_directory + 'msunet.%s.{epoch:02d}-{val_loss:.4f}.hdf5' % batch_size
    ...
    result_file = model_directory + 'results_%s.txt' % type_model
    ...
    model = get_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS, kernel_size)
    ...
    f.write("Batch size: {}\nMax Epochs: {}\nAccuracy: {:.2f}%\nLoss: {:.2f}%\nIoU: {:.3f}\nTraining time: {}\n\n".format(batch_size, num_epoch, acc_*100,loss_*100, iou_score, run_time))
    f.close()
    ...
    return [batch_size, num_epoch, acc_*100,loss_*100, iou_score, run_time]
...
model_directory = 'D:/Pycharm Projects/Cell Segmentation/4-model/%s/%s/test/' % (type_model, image_magnification)
table_file = model_directory + '/table_%s.txt' % type_model  

table = []

col_names = ['Batch size', 'Max Epochs', 'Accuracy', 'Loss', 'IoU', 'Training Time']

data = run_trials(data_directory, model_directory, kernel_size, image_magnification, type_model)
table.append(data)
...
for i, model_name in enumerate(models):  # Remember enumerate method adds a counter and returns the enumerate object
    if (model_name.split('.')[-1] == 'hdf5'):
        ...
        model_list.append(model_directory + model_name)

for x in range(len(model_list)-1):
    os.remove(model_list[x])